...
module.exports = {
    ...
    command_version: async function(messageFull,userID,userName,messageType,userRole){
        try {
            ...
            const replyFields = [
                [config.messages.version.botversion, String(config.bot.version), true],
                [config.messages.version.walletversion, String(walletInfo.version), true],
                [config.messages.version.walletprotocolversion, String(walletInfo.protocolversion || 0), true],
                [config.messages.version.walletconnections, String(walletInfo.connections || 0), true],
                [config.messages.version.walletblocks, String(walletInfo.blocks || 0), true],
                [config.messages.version.walletdifficulty, String(walletInfo.difficulty || 0), true]
            ];
    ...
    process_credit_stakes: async function(){
        try {
            ...
            console.log(`Credited ${stakeTransactions.length} stake transactions to ${creditedUsers} users`);
            return { 
                success: true, 
